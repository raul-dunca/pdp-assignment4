...
namespace ConsoleApp1
{
    public static class Program
    {
        ...
        {
            var entry = Dns.GetHostEntry(State.Host);                               // ipu lu www.cnatdcu.ro
            var socket = new Socket(SocketType.Stream, ProtocolType.Tcp);
            var endpoint = new IPEndPoint(entry.AddressList[0], State.Port);
            var state = new State(socket);
            state.Socket.BeginConnect(endpoint, ConnectCallback, state);            //state=socket +buffer  ; ConnectCallback se apeleaza dupa connect

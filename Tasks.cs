...
namespace ConsoleApp1
{
    public static class Program
    {
        public static void Main()
        {
            ...
            Task<int> future = Connect(state.Socket, endpoint, state);
            future.ContinueWith(task => ConnectCallback(state));
        ...
        private static void ConnectCallback(State ar)
        {
            var state = ar;
            ...
            Task<int> future = Send(state.Socket, requestBytes, 0, requestBytes.Length, state);
        ...
        private static void SendCallback(State ar)
        {
            var state = ar;
            state.SendDone.Set();

            Task<int> future = Receive(state.Socket, state.Buffer, 0, State.BufferLength, state);
            future.ContinueWith(receiveTask => ReceiveCallback(receiveTask, state));
        }
        ...
        private static void ReceiveCallback(Task<int> task, State s)
        {
            var state = s;
            var bytesReceived = task.Result;
            if (bytesReceived == 0)
            {
